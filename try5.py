...
# Compute the difference: only keep pixels where the modified image is darker than the original
darker_difference = np.where(modified < original, original - modified, 0).astype(np.uint8)

# Step 2: Normalize the darker difference image
normalized = cv2.normalize(darker_difference, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)

# Save the normalized darker difference to the same folder as the script
output_folder = os.path.dirname(__file__)
normalized_output_path = os.path.join(output_folder, "normalized_darker_difference.png")
cv2.imwrite(normalized_output_path, normalized)
print(f"Normalized darker difference saved to {normalized_output_path}")
...
# Iteratively enhance the darker difference
iterations = 4
...
# Original Darker Difference
plt.subplot(2, 3, 1)
plt.imshow(darker_difference, cmap='gray')
plt.title('Darker Difference Image')
plt.axis('off')

# Normalized Darker Difference
plt.subplot(2, 3, 2)
plt.imshow(normalized, cmap='gray')
plt.title('Normalized Darker Difference')
plt.axis('off')
...
output_image_path = os.path.join(output_folder, "final_enhanced_qr_code.png")
